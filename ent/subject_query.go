...
func (sq *SubjectQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, subject.UserTable, subject.UserColumn),
		)
...
// Returns a *NotSingularError when exactly one Subject entity is not found.
...
// Returns a *NotSingularError when exactly one Subject ID is not found.
...
func (sq *SubjectQuery) WithUser(opts ...func(*UserQuery)) *SubjectQuery {
	query := &UserQuery{config: sq.config}
...
//		GroupBy(subject.FieldSubjectID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	...
	if query := sq.withUser; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_subjects" returned %v`, n.ID)
			}
			for i := range nodes {
